...
const emojiArticleUser = require("../db/data/test-data/emojiArticleUser");
...
describe.skip("data insertion", () => {
    ...
    test("emoji data has been inserted correctly", () => {
    ...
    test("emoji article user data has been inserted correctly", () => {
        return db
            ...
            .then(({ rows }) => {
                ...
                rows.forEach((entry) => {
                    ...
                    expect(entry).toHaveProperty("emoji_id");
                    expect(typeof entry.emoji_id).toBe("number");

                    expect(entry).toHaveProperty("username");
                    expect(typeof entry.username).toBe("string");

                    expect(entry).toHaveProperty("article_id");
                    expect(typeof entry.article_id).toBe("number");
                });
            });
    });
    test("user topic data has been inserted correctly", () => {
        return db.query(`SELECT * FROM user_topic;`).then(({ rows }) => {
            expect(rows.length).toBeGreaterThan(0);

            rows.forEach((entry) => {
                expect(entry).toHaveProperty("user_topic_id");
                expect(entry).toHaveProperty("username");
                expect(typeof entry.username).toBe("string");
    ...
    test("user article votes data has been inserted correctly", () => {
        return db
            ...
            .then(({ rows }) => {
                ...
                rows.forEach((entry) => {
                    expect(entry).toHaveProperty("user_article_votes_id");
                    expect(entry).toHaveProperty("username");
                    expect(typeof entry.username).toBe("string");
